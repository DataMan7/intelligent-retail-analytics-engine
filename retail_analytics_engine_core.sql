...
-- 2.1: Create External Table for Product Images (Demo - using simulated data)
-- Note: In production, this would connect to actual image storage
-- CREATE EXTERNAL TABLE `retail_analytics.product_images`
-- OPTIONS(
...
-- Note: Model creation requires specific Vertex AI setup
-- CREATE OR REPLACE MODEL `retail_models.multimodal_embedding_model`
-- REMOTE WITH CONNECTION `projects/intelligent-retail-analytics/locations/us/connections/vertex-connection`
-- OPTIONS (
...
-- Note: Using simulated embeddings for demo - in production, use actual ML.GENERATE_EMBEDDING
...
SELECT
  ...
  p.category,
  p.price,
  -- Simulate text embeddings for demo
  [RAND(), RAND(), RAND(), RAND(), RAND()] as text_embedding,
  -- Simulate image embeddings for demo
  [RAND(), RAND(), RAND(), RAND(), RAND()] as image_embedding,
...
SELECT
  ...
  -- Note: AI functions require specific BigQuery AI setup
  -- AI.GENERATE_TEXT(
  --   'gemini-1.5-flash',
  ...
  CASE
    WHEN r.rating >= 4 THEN 0.8 + RAND() * 0.2
    WHEN r.rating >= 3 THEN 0.5 + RAND() * 0.3
    ELSE RAND() * 0.4
  END as sentiment_score_raw,
  ...
  -- ) as key_themes,
  -- Simulate themes for demo
  CASE
    WHEN r.rating >= 4 THEN 'quality,performance,value'
    WHEN r.rating >= 3 THEN 'features,usability,design'
    ELSE 'issues,problems,disappointment'
  END as key_themes,
  ...
  -- ) as review_category
  -- Simulate category for demo
  CASE
    WHEN r.rating >= 4 THEN 'QUALITY'
    WHEN r.rating >= 3 THEN 'SERVICE'
    ELSE 'QUALITY'
...
WITH review_aggregates AS (
  ...
  WHERE sentiment_score_raw IS NOT NULL
    AND REGEXP_CONTAINS(CAST(sentiment_score_raw AS STRING), r'^\d*\.?\d+$') -- Validate numeric
  GROUP BY product_id
),
sales_simulation AS (
...
SELECT
  ...
  CONCAT('Rating: ', CAST(COALESCE(ra.avg_rating, 0) AS STRING),
         ', Reviews: ', CAST(COALESCE(ra.total_reviews, 0) AS STRING),
         ', Category: ', p.category) as performance_insights
FROM `retail_analytics.products` p
...
-- Note: Vector indexes require specific BigQuery configurations
...
-- Note: Simplified version without vector search for demo
...
customer_embeddings AS (
  SELECT
    cph.customer_id,
    -- Note: Simplified customer preferences without embedding averaging
    NULL as preference_embedding,
    cph.total_purchases,
...
ai_insights AS (
  SELECT
    ...
    --          'Total Revenue: $', CAST(dm.total_revenue AS STRING),
    --          ', Products: ', CAST(dm.total_products AS STRING),
    --          ', Avg Rating: ', CAST(ROUND(dm.overall_rating, 2) AS STRING),
    --          ', High Performers: ', CAST(dm.high_rated_products AS STRING),
    --          ', Problem Products: ', CAST(dm.problematic_products AS STRING))
    -- ) as executive_summary,
    -- Simulate executive summary for demo
    CONCAT('Retail performance shows $', CAST(dm.total_revenue AS STRING),
           ' revenue with ', CAST(dm.total_products AS STRING), ' products') as executive_summary,
    ...
    -- Simulate strategic recommendations for demo
    'Focus on high-performing products and improve customer satisfaction' as strategic_recommendations
...
category_insights AS (
  SELECT
    ...
    --          ', Revenue: $', CAST(cm.category_revenue AS STRING),
    --          ', Avg Rating: ', CAST(ROUND(cm.category_rating, 2) AS STRING),
    --          ', Products: ', CAST(cm.product_count AS STRING),
    --          ', Top Products: ', COALESCE(cm.top_performers, 'None'))
    -- ) as optimization_strategies,
    -- Simulate strategies for demo
    CONCAT('Optimize pricing and improve product quality for ', cm.category) as optimization_strategies,
    -- Market positioning analysis
    -- Note: AI functions require specific BigQuery AI setup
    -- AI.GENERATE_TEXT(
    --   'gemini-1.5-flash',
    --   CONCAT('Assess market position for ', cm.category, ' category with avg price $',
    --          CAST(ROUND(cm.avg_price, 2) AS STRING), ' and rating ',
    --          CAST(ROUND(cm.category_rating, 2) AS STRING))
    -- ) as market_position
    -- Simulate market position for demo
    CONCAT(cm.category, ' has strong market position with ',
           CAST(ROUND(cm.category_rating, 1) AS STRING), ' rating') as market_position
...
WITH quality_analysis AS (
  SELECT
    ...
    --          ', Negative Reviews: ', CAST(pp.negative_reviews AS STRING),
    --          ', Rating: ', CAST(pp.avg_rating AS STRING),
    --          ', Category: ', pp.category)
    -- ) as root_cause_analysis,
    -- Simulate root cause for demo
    CONCAT('Quality issues identified for ', pp.product_name) as root_cause_analysis,
    -- Recommended actions
    -- Note: AI functions require specific BigQuery AI setup
    -- AI.GENERATE_TEXT(
    --   'gemini-1.5-flash',
    --   CONCAT('Suggest 3 specific actions to improve quality for: ',
    --          pp.product_name, ' with rating ', CAST(pp.avg_rating AS STRING),
    --          ' and ', CAST(pp.negative_reviews AS STRING), ' negative reviews')
    -- ) as recommended_actions
    -- Simulate actions for demo
    'Improve product quality and customer service' as recommended_actions
...
pricing_ai AS (
  SELECT
    ...
    --          'Current Price: $', CAST(ma.current_price AS STRING),
    --          ', Category Average: $', CAST(ROUND(ma.category_avg_price, 2) AS STRING),
    --          ', Rating: ', CAST(ma.avg_rating AS STRING),
    --          ', Sales Rank: ', CAST(ma.revenue_rank AS STRING), ' in category')
    -- ) as price_recommendation,
    -- Simulate price recommendation for demo
    CONCAT('Consider adjusting price based on market position for ', ma.product_name) as price_recommendation,
    ...
    -- Simulate promotional strategy for demo
    CASE
      WHEN ma.units_sold < 10 THEN CONCAT('Promote ', ma.product_name, ' with discounts')
      WHEN ma.avg_rating >= 4.5 THEN CONCAT('Position ', ma.product_name, ' as premium')
      ELSE CONCAT('Market ', ma.product_name, ' competitively')
    END as promotional_strategy
...
WITH customer_behavior AS (
  SELECT
    cr.customer_id,
    COUNT(DISTINCT cr.product_id) as products_reviewed,
    AVG(cr.rating) as avg_rating_given,
    STRING_AGG(DISTINCT p.category, ', ') as preferred_categories,
    -- Behavioral classification
    CASE
      WHEN AVG(cr.rating) >= 4.5 THEN 'SATISFIED'
      WHEN AVG(cr.rating) >= 3.5 THEN 'NEUTRAL'
      ELSE 'DISSATISFIED'
    END as satisfaction_level,
    CASE
      WHEN COUNT(DISTINCT cr.product_id) >= 10 THEN 'HEAVY_USER'
      WHEN COUNT(DISTINCT cr.product_id) >= 5 THEN 'REGULAR_USER'
      ELSE 'LIGHT_USER'
    END as usage_level
  FROM `retail_analytics.customer_reviews` cr
  JOIN `retail_analytics.products` p ON cr.product_id = p.product_id
...
segment_insights AS (
  SELECT
    ...
    --          ', Avg Rating: ', CAST(ROUND(cb.avg_rating_given, 1) AS STRING),
    --          ', Categories: ', cb.preferred_categories)
    -- ) as customer_strategy,
    -- Simulate customer strategy for demo
    CONCAT('Engage ', cb.satisfaction_level, ' customers with targeted offers') as customer_strategy,
    -- Personalized recommendations approach
    -- Note: AI functions require specific BigQuery AI setup
    -- AI.GENERATE_TEXT(
    --   'gemini-1.5-flash',
    --   CONCAT('Recommend personalization approach for customer who rates products at ',
    --          CAST(ROUND(cb.avg_rating_given, 1) AS STRING), ' stars and prefers ', cb.preferred_categories)
    -- ) as personalization_approach
    -- Simulate personalization for demo
    CONCAT('Personalize recommendations for ', cb.usage_level, ' users') as personalization_approach
...
WITH product_count AS (
  SELECT COUNT(*) as total_products FROM `retail_analytics.products`
),
review_count AS (
  SELECT COUNT(*) as total_reviews FROM `retail_analytics.customer_reviews`
),
embedding_count AS (
  SELECT COUNT(*) as embeddings_generated FROM `retail_analytics.product_embeddings`
),
avg_rating AS (
  SELECT AVG(avg_rating) as system_avg_rating FROM `retail_analytics.product_performance`
),
system_metrics AS (
  SELECT
    ...
    pc.total_products,
    rc.total_reviews,
    ec.embeddings_generated,
    ar.system_avg_rating
  FROM product_count pc
  CROSS JOIN review_count rc
  CROSS JOIN embedding_count ec
...
health_assessment AS (
  SELECT
    ...
    --          'Products: ', CAST(sm.total_products AS STRING),
    --          ', Reviews: ', CAST(sm.total_reviews AS STRING),
    --          ', Embeddings: ', CAST(sm.embeddings_generated AS STRING),
    --          ', Avg Rating: ', CAST(ROUND(sm.system_avg_rating, 2) AS STRING))
    -- ) as health_summary,
    -- Simulate health summary for demo
    CONCAT('System processing ', CAST(sm.total_products AS STRING),
           ' products with ', CAST(sm.total_reviews AS STRING), ' reviews') as health_summary,
...
BEGIN
  ...
  WITH high_risk_count AS (
    SELECT COUNT(*) as high_risk_products
    FROM `retail_insights.quality_alerts`
    WHERE quality_status = 'HIGH_RISK'
  ),
  critical_rating_count AS (
    SELECT COUNT(*) as critical_rating_products
    FROM `retail_analytics.product_performance`
  ...
  alert_data AS (
    SELECT
      ...
      hrc.high_risk_products,
      crc.critical_rating_products,
      trc.total_revenue
    FROM high_risk_count hrc
    CROSS JOIN critical_rating_count crc
  ...
  alert_summary AS (
    SELECT
      ...
      --          'High Risk Products: ', CAST(ad.high_risk_products AS STRING),
      --          ', Critical Ratings: ', CAST(ad.critical_rating_products AS STRING),
      --          ', Total Revenue: $', CAST(ad.total_revenue AS STRING))
      -- ) as executive_alert,
      -- Simulate executive alert for demo
      CONCAT('Alert: ', CAST(ad.high_risk_products AS STRING),
             ' high risk products identified') as executive_alert,
      CASE
...
trend_insights AS (
  SELECT
    ...
    --          ', Avg Price: $', CAST(ROUND(ta.avg_price, 2) AS STRING),
    --          ', Rating: ', CAST(ROUND(ta.avg_rating, 2) AS STRING),
    --          ', Sales: ', CAST(ta.total_sales AS STRING))
    -- ) as trend_analysis,
    -- Simulate trend analysis for demo
    CONCAT('Trend analysis for ', ta.category, ' category') as trend_analysis,
    -- Future predictions
    -- Note: AI functions require specific BigQuery AI setup
    -- AI.GENERATE_TEXT(
    --   'gemini-1.5-flash',
    --   CONCAT('Predict next quarter performance for ', ta.category,
    --          ' category with current sales of ', CAST(ta.total_sales AS STRING), ' units')
    -- ) as quarterly_forecast
    -- Simulate quarterly forecast for demo
    CONCAT('Forecast: ', ta.category, ' expected to grow by 15%') as quarterly_forecast
...
ai_recommendations AS (
  SELECT
    ...
    --          ' optimization affecting ', CAST(oa.affected_products AS STRING), ' products')
    -- ) as strategic_recommendation
    -- Simulate strategic recommendation for demo
